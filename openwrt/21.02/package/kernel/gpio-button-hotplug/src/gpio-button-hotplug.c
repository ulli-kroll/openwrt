...
static void gpio_keys_polled_check_state(struct gpio_keys_button_data *bdata)
{
	...
	if (state != bdata->last_state) {
		...
		if (state && (type != EV_SW)) {
			/* presse button, update timestamp */
			priv->seen = jiffies;
		}
		...
		priv->pressed_state = 0;
	} else {
		if (state && (type != EV_SW)) {
			unsigned long seen;
			seen = priv->seen;
			if (priv->pressed_state == 0) {
				if ((jiffies - priv->seen) / HZ >= 2) {
					/* pressed 2s */
					button_hotplug_event(bdata, type, state);
					priv->pressed_state = 1;
					priv->last_report = 2;
				}
			}

			if (priv->pressed_state == 1) {
				if (((jiffies - priv->seen) / HZ >= 5) && (priv->last_report == 2)) {
					/* pressed 5s */
					button_hotplug_event(bdata, type, state);
					priv->last_report = 5;
				}

				if (((jiffies - priv->seen) / HZ >= 15) && (priv->last_report == 5)) {
					/* pressed 15s */
					button_hotplug_event(bdata, type, state);
					priv->last_report = 15;
				}

				if (((jiffies - priv->seen) / HZ >= 60) && (priv->last_report == 15)) {
					/* pressed 1min */
					button_hotplug_event(bdata, type, state);
					priv->last_report = 60;
				}
