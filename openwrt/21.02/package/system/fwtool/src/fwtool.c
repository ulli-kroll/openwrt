...
static int seek_fitimage_offset(void)
{
	struct fdt_header * img_header;
	void *buf;
	int len;
#define BUF_BLOCK_LEN 2048

	buf = malloc(BUF_BLOCK_LEN);
	if (!buf)
		return -1;

	do {
		len = fread(buf, 1, BUF_BLOCK_LEN, firmware_file);
		img_header = (struct fdt_header *)(buf);
		if (fdt_check_header(img_header) == 0 ) {
			fseek(firmware_file, 0 - BUF_BLOCK_LEN, SEEK_CUR);
			free(buf);
			return 0;
		}
	} while (len == BUF_BLOCK_LEN);

	free(buf);
	return -1;	
}

static int seek_uimage_offset(void)
{
	image_header_t *img_header;
	void *buf;
	int len;

	buf = malloc(CHK_BLOCK_LEN);
	if (!buf)
		return -1;

	do {
		len = fread(buf, 1, CHK_BLOCK_LEN, firmware_file);
		img_header = (image_header_t *)(buf);
		if (UIMAGE_HEADER_MAGIC == ntohl(img_header->ih_magic)) {
			fseek(firmware_file, 0 - CHK_BLOCK_LEN, SEEK_CUR);
			free(buf);
			return 0;
		}
	} while (len == CHK_BLOCK_LEN);
...
extract_data(const char *name)
{
	...
	if (seek_uimage_offset() < 0) {
		msg("Failed to seek uimage offset, try check fitImage\n");

		//Recovery firmware_file position to SEEK_SET
		fseek(firmware_file, 0, SEEK_SET);
		if (seek_fitimage_offset() < 0) 
		{
			msg("Failed to seek fitImage offset\n");
			return 1;
		}
	}
