...
_ucidef_finish_switch_roles_dsa() {
	# inherited: $name $n_cpu $n_vlan $cpu0 $cpu1 $cpu2 $cpu3 $cpu4 $cpu5
	local index role roles num device need_tag want_untag port ports
...
ucidef_set_interface_macaddr_dsa() {
	local network="$1"
	local macaddr="$2"
...
ucidef_set_system_board() {
	local s_board=$1
	local s_model=$2
	local s_wan=$3
	local s_type=$4

	uci -q get system.board && return 0

	uci batch <<EOF
set system.board='board'
set system.board.model='$s_model'
set system.board.rom='$s_board'
set system.board.wan_port='$s_wan'
set system.board.def_type='$s_type'
set system.board.type='$s_type'
EOF
...
ucidef_add_switch_ext_powersave() {
	local device=$1
	local name=$2
	local port=$3
	local power_save_status=$4
	uci batch <<EOF
add network switch_ext
set network.@switch_ext[-1].device='$device'
set network.@switch_ext[-1].name='$name'
set network.@switch_ext[-1].port_id='$port'
set network.@switch_ext[-1].power_save_status='$power_save_status'
EOF
}

ucidef_add_switch_ext_mode() {
	local device=$1
	local name=$2
	local port=$3
	local mode=$4
...
ucidef_set_sfe_fastnat() {
	local sfe_support=$1
	local sfe_enable=$2

	uci -q get sfe.fastnat && return 0

	uci batch <<EOF
set sfe.fastnat=sfe
set sfe.fastnat.support=$sfe_support
set sfe.fastnat.enabled=$sfe_enable
EOF
