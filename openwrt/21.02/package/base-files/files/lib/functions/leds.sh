...
status_led_blink_failsafe() {
	led_timer $status_led 100 500
}

status_led_blink_preinit_regular() {
	led_timer $status_led 200 100
}

led_blink_upgrade() {
	local led_count=$(ls -al /sys/class/leds/ | grep -v hwdog | grep "^l" | wc -l)
	local led_delay_on=500
	let timer_count=led_count*1000
	let timer_count=timer_count-led_delay_on

	local ls_file=$(ls /sys/class/leds | grep -v hwdog)
