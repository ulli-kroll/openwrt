...
static int hnat_probe(struct platform_device *pdev)
{
	...
	err = of_property_read_u32_index(np, "mtketh-lan-gmac", 0, &val);
	if (err < 0)
	{
		g_lan_gmac = -1;
	}
	...
	{
		g_lan_gmac = val;
		dev_info(&pdev->dev, "mtketh-lan-gmac= %d\n", g_lan_gmac);
	}

	err = of_property_read_u32_index(np, "mtketh-wan-gmac", 0, &val);
	if (err < 0)
	{
		g_wan_gmac = -1;
	}
	...
	{
		g_wan_gmac = val;
		dev_info(&pdev->dev, "mtketh-wan-gmac= %d\n", g_wan_gmac);
	}
