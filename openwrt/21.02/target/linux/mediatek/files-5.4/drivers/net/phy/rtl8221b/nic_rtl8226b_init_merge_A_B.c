...
static struct phy_device *rtl8221b_phydev = NULL;
static unsigned int rtl8221b_phyaddr = 1;
...
{
	struct mii_bus *bus;
	if (rtl8221b_phydev == NULL)
		return 0;

	bus = rtl8221b_phydev->mdio.bus;
...
unsigned int mii_mgr_write(unsigned int phy_addr,unsigned int phy_register, u16 write_data)
{
	 struct mii_bus *bus;
	 if (rtl8221b_phydev == NULL)
	 	return 0;
...
static unsigned int rtl8221_phy_mmd_write_c22(unsigned int phy_id, u16 mmd_num, u16 reg_id, u16 reg_val)
{
	if (mmd_num == 31) {
		uint16_t reg;
		uint16_t dat;
		reg = reg_id/16;
		dat = 16 + (reg_id % 16) / 2;
		mii_mgr_write(phy_id, mmd_num, reg);
		mii_mgr_write(phy_id, dat, reg_val);
	 } else {
	 	uint16_t dat = 0x4000 | (mmd_num & 0x1F);
		mii_mgr_write(phy_id, 13, mmd_num);
		mii_mgr_write(phy_id, 14, reg_id);
		mii_mgr_write(phy_id, 13, dat);
		mii_mgr_write(phy_id, 14, reg_val);
	}

	return 0;
}

static unsigned int rtl8221_phy_mmd_write_c45(unsigned int phy_id, u16 mmd_num, u16 reg_id, u16 reg_val)
{
	mii_mgr_write(phy_id, mdiobus_c45_addr(mmd_num, reg_id), reg_val);
	
	return 0;
}

static unsigned int rtl8221_phy_mmd_read_c22(unsigned int phy_id, u16 mmd_num, u16 reg_id)
{
	...
	if (mmd_num == 31) {
		...
		dat = 16 + (reg_id % 16) / 2;
		mii_mgr_write(phy_id, mmd_num, reg);
		mii_mgr_read(phy_id, dat, &rdata);
	 } else {
	 	uint16_t dat = 0x4000 | (mmd_num & 0x1F);
		mii_mgr_write(phy_id, 13, mmd_num);
		mii_mgr_write(phy_id, 14, reg_id);
		mii_mgr_write(phy_id, 13, dat);
		mii_mgr_read(phy_id, 14, &rdata);
	 }

	return rdata;
}

static unsigned int rtl8221_phy_mmd_read_c45(unsigned int phy_id, u16 mmd_num, u16 reg_id)
{
	unsigned int rdata = 0;
	mii_mgr_read(phy_id, mdiobus_c45_addr(mmd_num, reg_id), &rdata);
...
MmdPhyWrite(
    ...
    IN UINT16 data)
{
	rtl8221_phy_mmd_write_c45(rtl8221b_phyaddr, dev, addr, data);
...
MmdPhyRead(
    ...
    OUT UINT16 *data)
{
	*data = rtl8221_phy_mmd_read_c45(rtl8221b_phyaddr, dev, addr);
    ...
    )
{
    ...
    const UINT16 patchver_a = 0x0021, patchver_b = 0x0020, patchaddr = 0x8024;
    ...
    if (VERID == 0x6700){
	   ...
	   //------------------------------------------------------------------------
        ...
        phydata &= 0xFFC0;
        phydata |= SDSMode;  //write bit [5:0] = 0/1/2/3
...
//ISRAM PATCH
  ...
  // Update patch version
		...
		printk("rtl8221b phy init (VERID=%02x)\n", VERID);
		status = Rtl8226b_serdes_autoNego_set(hDevice, 0);
        if (status != SUCCESS)
            goto exit;

		status = Rtl8226b_phy_reset(hDevice);
        if (status != SUCCESS)
            goto exit;

		status = Rtl8226b_PHYmodeEEE_set(hDevice, 1);
		if (status != SUCCESS)
			goto exit;

		//do not respond PHY address 0, PHY 0 is broadcast address
		MmdPhyRead(hDevice, MMD_VEND2, 0xA430, &phydata);
		phydata &= ~(BIT_13);
		MmdPhyWrite(hDevice, MMD_VEND2, 0xA430, phydata);
    }
...
static int rtl8221b_read(struct seq_file *m, void *private)
{
	pr_info("rtl8221b\n");
...
int dump_phyreg_read(struct seq_file *m, void *private)
{
	int i;
	u16 val = 0;
	HANDLE hDevice;
	...
	/* mmd, regid */
	u16 phyreg_info[16][2] = {
		/* disable serdes auto nego */
		{MMD_VEND1, 0x7588},
		{MMD_VEND1, 0x7589},
		{MMD_VEND1, 0x7587},
		/* disable phy 0 as broadcaset address */
		{MMD_VEND2, 0xA430},
		{0, 0}
	};

	printk("(mmd, regid) = result\n");
	for (i=0; phyreg_info[i][0]!=0; i++)
	{
		MmdPhyRead(hDevice, phyreg_info[i][0],  phyreg_info[i][1], &val);
		printk("(0x%x, 0x%x) = 0x%x\n", phyreg_info[i][0], phyreg_info[i][1], val);
	}

	return 0;
}

static int init_phy_read(struct seq_file *m, void *private)
{
	HANDLE hDevice;
    IN PHY_LINK_ABILITY *pphylinkability = NULL;
    BOOL singlephy = 0;
	
	Rtl8226b_phy_init(hDevice, pphylinkability, singlephy);
	return 0;
}

static int phyreset_read(struct seq_file *m, void *private)
{
	HANDLE hDevice;

	printk("rtl8221b reset\n");
	Rtl8226b_phy_reset(hDevice);
	return 0;
}

static int speed_read(struct seq_file *m, void *private)
{
	HANDLE hDevice;
	UINT16 speed = 0;
	UINT16 speed_index = 0;
	BOOL duplex_enable = FALSE;
	char *speed_str[] = {"None", "10M", "100M", "500M", "1000M", "2.5G"};
	char *duplex_str[] = {"half-duplex", "full-duplex", "None"};

	Rtl8226b_speed_get(hDevice, &speed);
	switch(speed)
	{
		case LINK_SPEED_10M:
			speed_index = 1;
	...
	seq_printf(m, "%s ", speed_str[speed_index]);

	Rtl8226b_duplex_get(hDevice, &duplex_enable);
	seq_printf(m, "%s ", duplex_str[duplex_enable]);

	return 0;
}

static int get_phyreg_open(struct inode *inode, struct file *file)
{
	return single_open(file, rtl8221b_read, file->private_data);
}

static ssize_t get_phyreg_write(struct file *file, const char __user *buf, size_t length, loff_t *offset)
{
	char line[64] = {0};
	size_t size;
	u16 val, mmd_num, reg_id;
	HANDLE hDevice;
	...
	if (sscanf(line, "%x %x", &mmd_num, &reg_id) != 2)
		return -EFAULT;

	line[length] = '\0';

	MmdPhyRead(hDevice, mmd_num,  reg_id, &val);
	printk("mmd_num=0x%x, reg_id=0x%x, val=0x%x\n", mmd_num, reg_id, val);
...
static ssize_t gmac_hotplug_write(struct file *file, const char __user *buf, size_t length, loff_t *offset)
{
	...
	gmac_hotplug_event(PHY_BASEADDR+rtl8221b_phyaddr, flag);
...
static int set_phyreg_open(struct inode *inode, struct file *file)
{
	return single_open(file, rtl8221b_read, file->private_data);
}

static ssize_t set_phyreg_write(struct file *file, const char __user *buf, size_t length, loff_t *offset)
{
	char line[64] = {0};
	size_t size;
	u16 mmd_num, reg_id, phydata;
	HANDLE hDevice;
	...
	if (sscanf(line, "%x %x %x", &mmd_num, &reg_id, &phydata) != 3)
		return -EFAULT;

	line[length] = '\0';

	MmdPhyWrite(hDevice, mmd_num, reg_id, phydata);
	printk("mmd_num=0x%x, reg_id=0x%x, phydata=0x%x\n", mmd_num, reg_id, phydata);
...
static int dump_phyreg_open(struct inode *inode, struct file *file)
{
	return single_open(file, dump_phyreg_read, file->private_data);
}

static int init_phy_open(struct inode *inode, struct file *file)
{
	return single_open(file, init_phy_read, file->private_data);
}

static int phyreset_open(struct inode *inode, struct file *file)
{
	return single_open(file, phyreset_read, file->private_data);
}

static int speed_open(struct inode *inode, struct file *file)
{
	return single_open(file, speed_read, file->f_path.dentry->d_iname);
}

static const struct file_operations get_phyreg_fops = {
	.open = get_phyreg_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = get_phyreg_write,
...
static const struct file_operations set_phyreg_fops = {
	.open = set_phyreg_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = set_phyreg_write,
	.release = single_release,
};

static const struct file_operations dump_phyreg_fops = {
	.open = dump_phyreg_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static const struct file_operations init_phy_fops = {
	.open = init_phy_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
...
static const struct file_operations phyreset_fops = {
	.open = phyreset_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
...
static int rtl8221b_init_debugfs(void)
{
	int ret = 0;
	struct dentry *root;

	root = debugfs_create_dir("rtl8221b", NULL);
	if (!root) {
		printk("%s:err at %d\n", __func__, __LINE__);
		ret = -ENOMEM;
		goto err0;
	...
	debugfs_create_file("get_phyreg", S_IRUGO | S_IRUGO, root, NULL, &get_phyreg_fops);
	debugfs_create_file("set_phyreg", S_IRUGO | S_IRUGO, root, NULL, &set_phyreg_fops);
	debugfs_create_file("dump_phyreg", S_IRUGO | S_IRUGO, root, NULL, &dump_phyreg_fops);
	debugfs_create_file("init_phy", S_IRUGO | S_IRUGO, root, NULL, &init_phy_fops);
	debugfs_create_file("phyreset", S_IRUGO | S_IRUGO, root, NULL, &phyreset_fops);
	debugfs_create_file("speed", S_IRUGO | S_IRUGO, root, NULL, &speed_fops);
...
int rtl8221b_phy_probe(struct phy_device *phydev)
{
	//unsigned int data = 0;
    HANDLE hDevice;
    IN PHY_LINK_ABILITY *pphylinkability = NULL;
	...
	rtl8221b_phydev = phydev;
	rtl8221b_phyaddr = phydev->mdio.addr;

	//Rtl8226b_phy_init(hDevice, pphylinkability, singlephy);

	//mii_mgr_read(rtl8221b_phyaddr, MII_BMCR, &data);
	//data |= BMCR_PDOWN;
	//mii_mgr_write(rtl8221b_phyaddr, MII_BMCR, data);

	rtl8221b_init_debugfs();
	return 0;
}

static struct phy_driver rtl8221b_phy_driver[] = {
	{
		PHY_ID_MATCH_MODEL(0x001cc849),
		.name		= "Realtek RTL8221B PHY",
		.probe		= rtl8221b_phy_probe,
	}, 
};

module_phy_driver(rtl8221b_phy_driver);

static struct mdio_device_id __maybe_unused rtl8221b_phy_tbl[] = {
	{ PHY_ID_MATCH_VENDOR(RTL_GENERIC_PHYID) },
	{ }
};

MODULE_DESCRIPTION("Realtek RTL8221B PHY driver");

MODULE_DEVICE_TABLE(mdio, rtl8221b_phy_tbl);
