...
static int mtk_probe(struct platform_device *pdev)
{
	...
	struct mtk_eth *eth;
	int err, i;
	struct mtd_info *lower = NULL;
	const char *mtdname = NULL;
	struct device_node *np = pdev->dev.of_node;

	err = of_property_read_string(np, "lower-mtd-name", &mtdname);
	if (!err) {
		lower = get_mtd_device_nm(mtdname);
		if (IS_ERR(lower))
		{
			dev_dbg(&pdev->dev, "failed to find mtd device by name '%s'\n", mtdname);
			/* retry probe to wait for mtd device, because gmac should get mac addr from "bdinfo" mtd partition */
			return -EPROBE_DEFER;
		}
