...
static unsigned int mt7981_phyaddr = 0;
...
static int mt7981_phy_read(struct seq_file *m, void *private)
{
	pr_info("mt7981_phy\n");
...
static int gmac_hotplug_open(struct inode *inode, struct file *file)
{
	return single_open(file, mt7981_phy_read, file->private_data);
}

static ssize_t gmac_hotplug_write(struct file *file, const char __user *buf, size_t length, loff_t *offset)
{
	char line[64] = {0};
	size_t size;
	u16 val, flag;
	...
	if (sscanf(line, "%x", &flag) != 1)
		return -EFAULT;

	line[length] = '\0';

	gmac_hotplug_event(PHY_BASEADDR+mt7981_phyaddr, flag);
...
static const struct file_operations gmac_hotplug_fops = {
	.open = gmac_hotplug_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = gmac_hotplug_write,
	.release = single_release,
};

static int mt7981_phy_init_debugfs(void)
{
	int ret = 0;
	struct dentry *root;

	root = debugfs_create_dir("mt7981_phy", NULL);
	if (!root) {
		printk("%s:err at %d\n", __func__, __LINE__);
		ret = -ENOMEM;
		goto err0;
	}
	debugfs_create_file("gmac_hotplug", S_IRUGO | S_IRUGO, root, NULL, &gmac_hotplug_fops);
...
{
	mt7981_phyaddr = phydev->mdio.addr;
	mt7981_phy_init_debugfs();
	return mt7981_phy_config_init(phydev);
}
