...
(
  ...
  while read F S; do
    echo "$SELF: $F: $S"
    # WAR: avoid strip wlan fw elf
    filename=`basename $F`
    [ "$filename" == "amss.bin" -o "$filename" == "m3.bin" -o "$filename" == "iu_fw.b02" -o "$filename" == "q6_fw1.b02" -o "$filename" == "amss_dualmac.bin" -o "$filename" == "q6_fw2.b02" ] && {
        echo "skip $F striping"
        continue
    }
	[ "${S}" = "relocatable" ] && {
		eval "$STRIP_KMOD $F"
