...
%_oemconfig: scripts/config/conf prepare-tmpinfo FORCE
	if [ \! -e $(TOPDIR)/oemconfig/$(@:_oemconfig=) ] ; then \
	    $(_SINGLE)$(NO_TRACE_MAKE) menuconfig $(PREP_MK); \
	fi
	$(TOPDIR)/scripts/oemconfig.sh $(TOPDIR)/oemconfig $(TOPDIR)/oemconfig/$(@:_oemconfig=) $(TOPDIR)/.config
