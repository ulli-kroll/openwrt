...
wps_pbc_enhc_overwrite_ap_settings() {
	...
	if [ -r $WPAD_VARRUN/hostapd-${parent_overwrite}/${ifname_overwrite} ]; then
		hostapd_cli -i${ifname_overwrite} -p$WPAD_VARRUN/hostapd-${parent_overwrite} wps_config \
...
wps_pbc_enhc_overwrite_sta_settings() {
	...
	if [ -r $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} ]; then
		nw=`wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} add_network | cut -d ' ' -f 4`
		wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw ssid \"${ssid_overwrite}\"
		uci set wireless.${sect}.ssid="$ssid_overwrite"
		case $auth_overwrite in
			WPA3-SAE)
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw key_mgmt SAE
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw ieee80211w 2
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw sae 1
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw auth_alg OPEN
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw pairwise CCMP
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw proto RSN
				if [ $key_overwrite_len -eq 64 ]; then
					wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw psk ${key_overwrite}
				else
					wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw psk \"${key_overwrite}\"
				fi
			...
			;;
                        WPA3-PSK)
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw key_mgmt WPA-PSK SAE
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw ieee80211w 1
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw sae 1
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw auth_alg OPEN
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw pairwise CCMP
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw proto RSN
			...
			WPA2-PSK)
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw key_mgmt WPA-PSK
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw auth_alg OPEN
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw pairwise CCMP
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw proto RSN
				if [ $key_overwrite_len -eq 64 ]; then
			...
			WPA-PSK)
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw key_mgmt WPA-PSK
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw auth_alg OPEN
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw pairwise TKIP
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw proto WPA
				if [ $key_overwrite_len -eq 64 ]; then
					wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw psk ${key_overwrite}
				else
					wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw psk \"${key_overwrite}\"
			...
			;;
			NONE)
				wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} set_network $nw key_mgmt NONE
				uci set wireless.${sect}.encryption='none'
				uci set wireless.${sect}.key=''
			;;
		esac
		wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} enable_network $nw
		wpa_cli -p $WPAD_VARRUN/wpa_supplicant-${ifname_overwrite} save_config
	fi
}
...
case "$CMD" in
	CONNECTED)
		...
		[ -e $WPAD_VARRUN/wps-hotplug-$IFNAME.pid ] && kill "$(cat "$WPAD_VARRUN/wps-hotplug-$IFNAME.pid")"
