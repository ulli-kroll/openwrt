			...
			wrap|ap)
                ...
                if [ -z mld ]; then
                    ...
                    [ -z "$extender_device" ] || {
                        ...
                        config_get mld_ref "$mld" mld_ref
                        [ -z "$mld_ref" ] || mld_ref_option="mld_id"

                        wrapd -S -g $wrapd_global_ctrl_iface ap_add $extender_device $vifname $conf $cmd_type $mld_mac_option $mld_mac $mld_ref_option $mld_ref
                    }
                fi
			;;
		esac
		...
		case "$mode" in
			sta)
                ...
                if [ -z mld ]; then
                    ...
                    wrapd -S -g $wrapd_global_ctrl_iface sta_add $device $vifname $tmp_conf_file $cmd_type $mld_mac_option $mld_mac $mld_ref_option $mld_ref
                fi
...
qwrap_setup() {
    ...
    wrapd -S -g $wrapd_global_ctrl_iface -W
	return 1
}
