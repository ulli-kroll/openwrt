#!/bin/sh /etc/rc.common
# Copyright (c) 2015, 2017-2019, 2021 Qualcomm Technologies, Inc.
#
# All Rights Reserved.
# Confidential and Proprietary - Qualcomm Technologies, Inc.
#
# 2015 Qualcomm Atheros, Inc.
#
# All Rights Reserved.
# Qualcomm Atheros Confidential and Proprietary.


# shellcheck disable=SC2034
START=97  # needs to be after LED init

# shellcheck disable=SC2034
SERVICE_WRITE_PID=1

# shellcheck disable=SC2034
SERVICE_DAEMONIZE=1

#SERVICE_DEBUG=1
NETWORK_RESTART=0
SERVICE_DEBUG_OUTPUT=0

# These restart_in_*_mode commands are only intended to be used by
# repacd-run.sh.
#
# - restart_in_cap_mode is used when the device has a direct connection to the
#   the gateway via ethernet. This will result in only the AP interfaces being
#   enabled.
# - restart_in_noncap_mode is used when the device no longer has a direct
#   connection to the gateway via ethernet. When this is done, the device will
#   either have both its AP and STA interfaces enabled (if its primary purpose
#   is as a standalone RE) or just its STA interface enabled (if its primary
#   purpose is as a client device).
# - restart_in_re_mode is used when a device that is intended to primarily
#   act as a client actually has sufficient link quality to act as a range
#   extender. It will enable both the STA and AP interfaces.
EXTRA_COMMANDS="restart_in_cap_mode restart_in_noncap_mode restart_in_re_mode update_bss_config update_wireless_vap_parameters update_ingress_egress_priority repacd_set_map_dpp_config config_agent_only_mode"

EXTRA_HELP=<<EOF
        restart_in_cap_mode Reconfigure the system into Central AP mode
        restart_in_noncap_mode Reconfigure the system into Non Central AP mode
        restart_in_re_mode Reconfigure the system into Range Extender mode
EOF

. /lib/functions/repacd-cmn.sh

#Creating a tmp file to identify the first boot of repacd
echo 1 > /tmp/firstboot

#check repacd config to enable/disable cfg80211
config_load 'repacd'
config_get_bool map_enabled MAPConfig 'Enable' '0'
config_get_bool repacd_enabled repacd 'Enable' '0'
config_get_bool enable_yocto_build MAPConfig 'EnableYoctoBuild' '0'
    if [ "$map_enabled" -eq 1 ]; then
         . /lib/functions/repacd-map.sh
    elif [ "$repacd_enabled" -eq 1 ]; then
         . /lib/functions/repacd-son.sh
    fi

    if [ "$enable_yocto_build" -eq 1 ]; then
         brprefix=''
         primarybridgename='bridge0'
    else
         brprefix='br-'
         primarybridgename='lan'
    fi

board_name=""
[ -f /tmp/sysinfo/sdk_board_name  ] && {
        board_name=ap$(cat /tmp/sysinfo/sdk_board_name | awk -F 'ap' '{print$2}')
    }

# Script entry point: Perform configuration and start the daemon
start() {
    local enabled

    config_load 'repacd'
    config_get_bool enabled repacd 'Enable' '0'

    [ -e /tmp/bootup_completed ] || touch /tmp/bootup_completed

    [ "$enabled" -gt 0 ] || {
        return 1
    }

    if __repacd_is_map_enabled; then

        if __repacd_is_map_lite_enabled; then
            __start_map_lite
            return $?
        fi

        __repacd_map_reset_5g_attempts

        local ether_gwcon=0 start_role='init' autoconf=0
        __start_map $ether_gwcon $start_role $autoconf
        return $?
    else
        __start_son
        return $?
    fi
}

# Script entry point: Stop the daemon
stop() {
    __stop_repacd_run
}

# Script entry point: Reconfigure and restart the daemon
restart() {
    stop

    config_load 'repacd'
    config_get_bool enabled repacd 'Enable' '0'

    [ "$enabled" -gt 0 ] || {
            return 1
    }

    start
}

# Force a restart into CAP mode.
#
# This is used when the gateway detection logic detects a gateway on
# ethernet when running in a pure bridge mode.
restart_in_cap_mode() {
    if __repacd_is_map_enabled; then
        __restart_in_cap_mode_map
        return $?
    else
        __restart_in_cap_mode_son
        return $?
    fi
}

# Force a restart into NonCAP mode.
#
# This is used when the gateway detection logic detects that there is no
# longer a gateway connected over ethernet.
restart_in_noncap_mode() {
    if __repacd_is_map_enabled; then
        __restart_in_noncap_mode_map
        return $?
    else
        __restart_in_noncap_mode_son
        return $?
    fi
}

# Force a restart into Range Extender (RE) mode.
#
# This is used when the Wi-Fi link monitoring logic determines that the
# link is in the sweet spot for a device that normally acts as a client
# but is capable (from a CPU and Wi-Fi perspective) of operating as an AP
# and a STA at the same time.
restart_in_re_mode() {
    if __repacd_is_map_enabled; then
        __restart_in_re_mode_map
        return $?
    else
        __restart_in_re_mode_son
        return $?
    fi
}

#This Function is Used to invoke DPP related hostapd calls for Key Generation
repacd_set_map_dpp_config() {
    config_get map_version MAPConfig 'MapVersionEnabled' '1'
    config_get onboarding_type MAPConfig 'OnboardingType'

    if [ "$onboarding_type" == "dpp" ]; then
        map_dpp_enabled=1
    fi
    if [ "$map_version" -ge 3 -a "$map_dpp_enabled" -eq 1 ]; then
        local dpp_config_key
        local dpp_key_len
        local gwcon_mode is_controller
        local start_role='init'

        if __repacd_gw_mode || [ "$gwcon_mode" = 'Controller' ]; then
            is_controller=1
        fi

        # Delete temp files
        rm /tmp/map_key_info.tmp
        rm /tmp/mapConfig.log

        # Get Configurator key if already present
        dpp_config_key=$(cat /etc/map_dpp_key)
        dpp_key_len=${#dpp_config_key}

        if [ "$is_controller" -eq 1 ]; then
            __repacd_set_map_dpp_config $is_controller $start_role
            return $?
        fi
    fi
}

#update bss_policy.conf file
#
#This is used to update the bss_policy.conf file after changing the
#VAP config options ex: SSID, Key, authtype.
update_bss_config() {
    local config_changed=0

    __repacd_generate_map_bss_conf config_changed
    return $?
}

update_wireless_vap_parameters() {
    local config_changed=0

    # This call is needed to update the ifname info in the wireless
    # especially in case of yocto, when SON scripts try to fetch the
    # iface value from ifname it always gets returned as empty.
    if [ "$enable_yocto_build" -eq 1 ]; then
        /sbin/wifi scan
    fi

    __repacd_update_vap_parameters
    __repacd_generate_map_bss_conf config_changed
    return $?
}

update_ingress_egress_priority() {
    config_load repacd
    config_get map_backhaul_nw MAPConfig 'VlanNetworkBackHaul' ''
    config_get num_vlan_supported MAPConfig 'NumberOfVLANSupported' '0'

    __repacd_map_set_egress_ingress_ports
}

__repacd_coloc_config_agent_only_mode_usage() {

    echo
    echo "Usage:  /etc/init.d/repacd config_agent_only_mode [OPTION] [<arguments>]"
    echo
    echo "OPTIONS:"
    echo
    echo "    --create_default_vap      - create required initial vap for mesh"
    echo "    --create_default_bridges  - create required bridges for traffic seperation in R2"
    echo "    --create_vlan             - create virtual lan and add it to respective bridge"
    echo
}

# Support for external EM Controller.
# QCA Co-located Agent configuration is handled below for EXT EM Controller

# Flow
# --create_default_vap     - Create default vaps and virtual Vap for communicate with
#                            third party conroller.
#                            Virtual vap will be added in hyctl loop back interface.
# --create_default_bridges - Create bridges according to the number of vlan supported
# --create_vlan            - Apply Vlan ID for respective vaps and ethernet according to
#                            the traffic seperation policy.
# Note: --create_vlan will be called from wsplcd when the agent gets M2 message.
config_agent_only_mode() {

. /lib/functions/repacd-map.sh

    local is_controller=1

    option="${1}"
    case ${option} in
        "--create_default_vap")
                create_default_vap $is_controller
                ;;
        "--create_default_bridges")
                create_default_bridges $is_controller
                ;;
        "--create_vlan")
                create_vlan
                ;;
        *)
                echo "ERROR: $0 Terminated - Invalid argument"
                __repacd_coloc_config_agent_only_mode_usage
                exit 1
    esac
}
