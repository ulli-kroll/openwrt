...
class MbnHdrV6(object):
    ...
    QTI_VERSION_TO_METADATA_MAP = {
        (METADATA_MAJOR_VERSION_QTI_0, METADATA_MINOR_VERSION_QTI_0):
            METADATA_0_0_QTI}
    VERSION_TO_METADATA_MAP = {
        (METADATA_MAJOR_VERSION_0, METADATA_MINOR_VERSION_0):
            METADATA_0_0}
    QTI_VERSION_TO_FLAGS_MAP = {
        (METADATA_MAJOR_VERSION_QTI_0, METADATA_MINOR_VERSION_QTI_0):
            FLAGS_O_O_QTI}
    VERSION_TO_FLAGS_MAP = {
        (METADATA_MAJOR_VERSION_0, METADATA_MINOR_VERSION_0):
            FLAGS_O_O}
                ...
                if field_name in metadata.keys():
                    if value is None:
                        ...
                        raise RuntimeError(
                            ...
                            "it is not of type int, str, or list. Type: {2}"
                            .format(field_name, authority,
                                    metadata[field_name].__class__.__name__))
    ...
    def get_metadata_qti(self):
        return getattr(self, "metadata_qti", None)
    ...
    def validate(self):
        ...
        if bin_version != (None, None) and bin_version != version:
            raise RuntimeError(
                'MBN OEM metadata version {0}.{1} does not match expected '
                'version {2}.{3}.'.format(
                    bin_version[0], bin_version[1], version[0], version[1]))
        bin_version = self.get_metadata_version_qti_in_bin()
        version = self.get_metadata_version_qti()
        if bin_version != (None, None) and bin_version != version:
            raise RuntimeError(
                'MBN QTI metadata version {0}.{1} does not match expected '
                'version {2}.{3}.'.format(
                    bin_version[0], bin_version[1], version[0], version[1]))
