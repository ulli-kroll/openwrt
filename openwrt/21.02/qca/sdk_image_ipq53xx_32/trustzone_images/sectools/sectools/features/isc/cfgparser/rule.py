...
class _ImageList(object):
    ...
    """
    def __init__(self, verbatim_config=False):
    ...
    def validate(self, images, *args):
        ...
        for image in images.get_image():
            ...
            if (attributes.num_root_certs > 1 and
                ...
                add_error(sign_id, "Multiple root certs are not supported for secboot {0} chipset {1}."
    ...
    def sanitize_soc_vers(self, sign_id, attributes, add_error):
        ...
        if attributes.secboot_version == SECBOOT_VERSION_3_0:
            if attributes.hmac:
                add_error(sign_id, "Secboot version 3.0 does not support hmac.")
            if attributes.mask_soc_hw_version is not None:
                add_error(sign_id, "Secboot version 3.0 does not support mask_soc_hw_version")
        else:
        ...
        if self.verbatim_config:
            ...
            # Make sure there are soc_ver_candiates when in_use_soc_hw_version is enabled
...
class ConfigRulesManager(CoreRuleBase):
    ...
    def __init__(self, verbatim_config=False):
        ...
        self.configs['images_list'] = _ImageList(verbatim_config=verbatim_config)
