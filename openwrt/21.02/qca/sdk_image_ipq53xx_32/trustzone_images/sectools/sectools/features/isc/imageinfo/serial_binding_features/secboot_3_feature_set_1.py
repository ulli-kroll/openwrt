...
ALLOWED_CHIPSETS = ("sdx24", "sdx55", "sm8150", "sm8250", "sm6150", "sm7150",
                    "sc8180x", "qcs405", "nicobar", "9205", "saipan", "rennell")
...
class Secboot3FeatureSet1(RotEnNotSupported,
    ...
    def bind(cls, config):
        ...
        The affected attributes are:
          * debug - only 0, 1 or 2 without the serial number
        ...
        """
        config.general_properties.debug = "0x%016X" % (
                int(config.general_properties.debug, 16) & 0x00000000FFFFFFFFL)
    ...
    def is_matched(self, config):
        if super(Secboot3FeatureSet1, self).is_matched(config):
            return config.general_properties.secboot_version == SECBOOT_VERSION_3_0
        return False
    ...
    def max_serial_number_count(cls):
        return MAX_NUM_SERIALS_MAP[SECBOOT_VERSION_3_0]
