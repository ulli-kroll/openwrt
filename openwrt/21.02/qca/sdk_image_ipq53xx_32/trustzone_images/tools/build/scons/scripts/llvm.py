...
#
# Copyright (c) 2009-2017,2019,2022 by Qualcomm Technologies, Incorporated.
# All Rights Reserved.
# QUALCOMM Proprietary/GTDR
#
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/scons.tzw/1.0/build/scons/scripts/llvm.py#4 $
#  $DateTime: 2022/09/06 03:29:21 $
#  $Change: 39504357 $
...
# --------   ---     ---------------------------------------------------------
#9/6/22    mp      Fix linker warnings due to memmove/memsmove
#11/18/19  mp      Fix compiler warnings
...
def generate(env):
   ...
   if do_64:
      ...
      LLVMCPPFlags = " ".join(llvm_cpp_flags_tz)
      if use_clang_assembler:
         env.Replace(AS = LLVMBIN + "clang${EXE_EXT} -target arm-linux-gnu -mfloat-abi=softfp -mcpu=cortex-a8 -Wno-unused-command-line-argument -x assembler -allow-integrated-as-for-asm-input -c ")
   ...
   sym_bld2 = Builder(action='$NM -f $SOURCE -o $TARGET && sed -i "/^mems*move/d" $TARGET ',suffix='.sym',src_suffix='.tempsym')
