...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/scons.tzw/1.0/build/scons/scripts/config.py#3 $
#  $DateTime: 2020/01/02 22:02:52 $
#  $Author: pwbldsvc $
#  $Change: 22041630 $
...
# --------   ---     ---------------------------------------------------------
# 01/02/20   as      Compiler warnings fix
# 05/15/09   wd      Create
#
#===============================================================================
import os
...
def publish_invalid_public_api(env, api_name, paths):
   env.PrintInfo("Invalid environment context for PublishPublicApi \n" \
...
def publish_invalid_restricted_api(env, api_name, paths):
   env.PrintInfo("Invalid environment context for PublishRestrictedApi \n" \
                 "   called from [" + inspect.stack()[2][1] + "]")
   return None
...
def publish_invalid_protected_api(env, api_name, paths):
   env.PrintInfo("Invalid environment context for PublishProtectedApi \n" \
                 "   called from [" + inspect.stack()[2][1] + "]")
   return None
...
def publish_invalid_private_api(env, api_name, paths, auto_require=True):
   env.PrintInfo("Invalid environment context for PublishPrivateApi \n" \
                 "   called from [" + inspect.stack()[2][1] + "]")
   return None
