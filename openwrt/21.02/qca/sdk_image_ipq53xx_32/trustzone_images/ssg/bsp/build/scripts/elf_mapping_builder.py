...
def elf_mapping_builder (target, source, env):
  ...
  pt_pool_size = pt_pool_info[0]['size']
  pt_pool_offset_in_elf = 0
  ...
  for i in range(elf_header.e_phnum):
    ...
    if is_valid_segment(phdr_entry) and phdr_entry.p_memsz > 0:
      # Search for page table memory pool location/offset in elf.
      if pt_pool_addr >= phdr_entry.p_paddr and pt_pool_addr < (phdr_entry.p_paddr + phdr_entry.p_memsz):
        pt_pool_offset_in_elf = (pt_pool_addr - phdr_entry.p_paddr) + phdr_entry.p_offset
  ...
  pt_data_levelwise = ['' for x in arch.mapping_sizes]
  pt_addr_levelwise = [[] for x in arch.mapping_sizes]
  pt_data_total_size = 0

  for x in range (len(arch.mapping_sizes)):
    pt_addr_levelwise[x] = pt_pool_addr
    pt_pool_addr += (arch.table_sizes[x] * len(arch.ptables[x]))

  if 32 == arch.ver:
    generate_ptables_armv7_short_desc(pt, 0, pt_addr_levelwise, pt_data_levelwise)
  else:
    generate_ptables_armv8_long_desc(pt, 0, pt_addr_levelwise, pt_data_levelwise, arch)
  ...
  for i in range(len(pt_data_levelwise)):
    if (len(pt_data_levelwise[i]) % arch.table_sizes[i]):
      diff =  arch.table_sizes[i] - (len(pt_data_levelwise[i]) % arch.table_sizes[i])
      pt_data_levelwise[i] += ((diff / len(arch.empty_pt_marker)) * arch.empty_pt_marker)
    pt_data_total_size += len(pt_data_levelwise[i])


  if pt_data_total_size > pt_pool_size:
    print "Fatal Error!! PT Memory Overflow"
    return -1

  diff = pt_pool_size - pt_data_total_size
  if diff:
    pt_data_levelwise[len(pt_data_levelwise) - 1] += ((diff / len(arch.empty_pt_marker)) * arch.empty_pt_marker)

  elf_fp = open (elf_file_name, 'r+b')
  elf_fp.seek(pt_pool_offset_in_elf)
  for i in range(len(pt_data_levelwise)):
    elf_fp.write(pt_data_levelwise[i])
  elf_fp.close()
