...
TA_SYMBOL_EXPORT const size_t ta_minExecTimeout = {minExecTimeout};

TA_SYMBOL_EXPORT const size_t ta_minWaitTimeout = {minWaitTimeout};
...
def add_secure_app_metadata(env, target,source):
   ...
   with file(out_path, 'w') as output:
      ...
      metadata_minExecTimeout = SecureAppMetadata.extractField(md, 'minExecTimeout')
      metadata_minWaitTimeout = SecureAppMetadata.extractField(md, 'minWaitTimeout')
      ...
      whole_file = ENTIRE_FILE_FORMAT_STRING.format(
          ...
          minExecTimeout   = metadata_minExecTimeout,
          minWaitTimeout   = metadata_minWaitTimeout,
          )
