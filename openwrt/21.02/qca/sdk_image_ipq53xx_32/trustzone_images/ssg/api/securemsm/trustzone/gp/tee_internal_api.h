...
/*------------------------------------------------------------------------------
      Internal Core API Extension Functions
  ------------------------------------------------------------------------------*/
/**
 * @brief Copy bytes from source to destination, with bounds checking.
 *
 * The TEE_MemSMove_ext function copies bytes from the object pointed to by src into the object pointed to by dest.
 * The number of bytes copied is the minimum of srcSize and destSize if both buffer pointers are non-NULL,
 * or zero otherwise
 *
 * Copying takes place as if the bytes from the object pointed to by src are first copied into a temporary
 * array that does not overlap the objects pointed to by dest and src, and then the size bytes
 * from the temporary array are copied into the object pointed to by dest.
 *
 * @param[out]  dest      A pointer to the destination buffer
 * @param[in]   destSize  The destination buffer size
 * @param[in]   src       A pointer to the source buffer
 * @param[in]   srcSize   The source buffer size
 *
 * @return The number of bytes copied
 */
extern size_t TEE_MemSMove_ext(
    void       *dest,
    size_t      destSize,
    const void *src,
    size_t      srcSize);
...
/**
 * @brief Compare two blocks of memory, with bounds checking.
 *
 * The TEE_MemSCompare function compares the bytes of the object pointed to by buffer1 to the bytes of the
 * object pointed to by buffer2. The number of bytes compared
 *
 * @param[in] buffer1     A pointer to the first buffer
 * @param[in] buffer1Size The size of the first buffer
 * @param[in] buffer2     A pointer to the second buffer
 * @param[in] buffer2Size The size of the second buffer
 *
 * @return The sign of a non-zero return value is determined by the sign of the difference between the values of
 *         the first pair of bytes (both interpreted as type uint8_t) that differ in the objects being compared.
 *           If the first byte that differs is higher in buffer1, then return a small positive value.
 *           If the contents of the two buffers are identical, then return zero.
 *           If the first byte that differs is higher in buffer2, then return a small negative value.
 *           If either buffer pointer is NULL, or the two sizes differ, then return INT32_MIN.
 */
extern int32_t TEE_MemSCompare_ext(
    const void *buffer1,
    size_t      buffer1Size,
    const void *buffer2,
    size_t      buffer2Size);
