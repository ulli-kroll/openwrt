...
#deploy tree
#TODO: since we're deploying a shit load of files I've temporarily upped this a lot
#fix once Dubai's changes are in
C_FILE_THRESHOLD = 10000
...
def allow_deploy(file_path):
  ''' Helper function to determine if we should block deployment
      of a file.  This is to prevent deploying unwanted classes of
      files, such as intermediate compiler files.'''
...
def deploy_generate(env):
  ...
  env.AddMethod(get_deploy_root, 'GetDeployRoot')
  env.AddMethod(nuke_deploy_tree, "NukeDeployTree")
...
def _deploy_common(env, deploy_list, default_variants, **kwargs):
  ...
  deploy_units = deploy_builder(env, deploy_list, deploy_variants, kwargs.get('deploy_target'), kwargs.get('path_map'))
  return deploy_units
...
def deploy_builder(env, deploy_list, deploy_variants, deploy_target, path_map):
  '''
  ...
  '''
  #some items in deploy_list may have been passed as strings with substitutions that
  #ROOT_GLOBAL_ENV doesnt have, so lets subst all of them here
  deploy_list = [env.subst(s) for s in env.Flatten(deploy_list)]
  if 'deploy' in COMMAND_LINE_TARGETS or 'deploy_sdk' in COMMAND_LINE_TARGETS:
    return process_deploy_command(env['ROOT_GLOBAL_ENV'], deploy_list, deploy_variants, deploy_target, path_map)

def nuke_deploy_tree(env, variant=''):
  ''' Nuke the deploy tree if it is present'''
  variant_tree = os.path.join(get_deploy_root(env),variant)
  ...
  if os.path.exists(variant_tree):
    remove_tree(variant_tree)
    print("DeployBuilder: NukeDeployTree {}".format(variant_tree))
  else:
    print("DeployBuilder: NukeDeployTree {} does not exist".format(variant_tree))
  return variant_tree

def process_deploy_command(env, deploy_list, deploy_variants, deploy_target, path_map):
  ...
  requested_variants = deploy_variants
  if not requested_variants:
    requested_variants = DeployVariant.get_default_public_variants()
  ...
  node_list = []
  for item in dlist:
    item_path = env.SConsAbsPath(item)
    if allow_deploy(item_path):
      node_list.append(item_path)

  deploy_target = deploy_target if deploy_target else 'deploy'
  obj = deploy_command(env, dv, workspace_root, deploy_root, node_list, path_map, deploy_target)
  return obj
...
def get_command_line_variant(env):
  ...
  '''
  cv = []
  dv = ARGUMENTS.get('deploy_variant')
  if dv:
    if isinstance(dv,list):
      cv = [d.upper() for d in dv]
    else:
      cv = [dv.upper()]
  else:
...
def get_deploy_root(env):
  ...
  '''
  if not ARGUMENTS.get('deploy_root'):
    #construct the path
    #qcscons is stupid and appends to the buildid, so let's undo that
    build_id = env.subst('${BUILD_ID}') [0:8]
    return os.path.join(os.path.dirname(ammend_file_path(env.subst('${BUILD_ROOT}'))),
                        'deploy_' + build_id)

  return ARGUMENTS.get('deploy_root')

def deploy_command(env, variants, workspace_root, deploy_root, node_list, path_map, deploy_target):
  '''
  ...
  for v in variants:
    ...
    for k, v in target_source_dict.items():
      target.append(v)
      r = env.InstallAs(v,k)
      env.Alias(deploy_target, r)
    target_list.append(target)
  return target_list

def _deploy_dir(env, dir_root, excludes=None):
  '''helper function that walks a subtree and Deploys every file in that tree'''
  if excludes is None:
    excludes = []
  excludes = [env.RealPath(s) for s in excludes]
  for (path, dname, fname) in os.walk(env.RealPath(dir_root)):
    for j in fname:
      full_path = env.RealPath(os.path.join(path,j))
      if allow_deploy(full_path):
        if not any([e in full_path for e in excludes]):
          env.Deploy(full_path)
        else:
          print("excluding {} because it's in deploy dir excludes".format(full_path))
...
class VerifyBuild(object):
  ...
  def verify_compilation(self, variant):
    ...
    return_val = True
    print 'verifycompilation'
    try:
      branch = ARGUMENTS["SOFTWARE_IMAGE"]
      build_cmd = 'python build_all.py --fast CHIPSET={} -b {} --cfg {} '.\
                                               format(
                                                 self.chipset, \
                                                 branch, \
      ...
      process = subprocess.Popen(build_args,cwd=build_dir, stdout=sys.stdout, stderr=sys.stderr)
