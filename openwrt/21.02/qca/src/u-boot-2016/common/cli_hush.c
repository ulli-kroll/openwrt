...
{
        int leds_node = -1, watchdog_node = -1;
	const fdt32_t *gpio_prop;
        int watchdog_gpio_pin;
	ulong value;

        leds_node = fdt_path_offset(gd->fdt_blob, "/leds");
        if (leds_node < 0) {
                return;
        }

	watchdog_node = fdt_subnode_offset(gd->fdt_blob, leds_node, "hw_watchdog");
	if (watchdog_node < 0)
	{
		return;
	}

	gpio_prop = fdt_getprop(gd->fdt_blob, watchdog_node, "gpio", NULL);
	if (!gpio_prop)
	{
		return;
	}

	watchdog_gpio_pin = fdt32_to_cpu(gpio_prop[0]);

	value = gpio_get_value(watchdog_gpio_pin);
	gpio_set_value(watchdog_gpio_pin, !value);
