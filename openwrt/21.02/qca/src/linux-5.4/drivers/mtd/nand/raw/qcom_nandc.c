...
#define SPI_FLASH_OTP_ENABLE		(1 << 6)
...
extern u8* nand_get_flash_id(void);
extern int flash_uuid_init(u8 *uuid, int len);
/*
static int GD5F1GQ5_nandflash_uid_detect(struct nand_chip *chip, u8* uuid);
static int F50D1G41LB_nandflash_uid_detect(struct nand_chip *chip, u8* uuid);
*/
static int nandflash_uid_detect_common(struct nand_chip *chip, u8* uuid, int page_addr);
...
struct nand_flash_dev_uid {
	...
	uint16_t id_len;
	int page_addr;
	int (*flash_uid_detect)(struct nand_chip *chip, u8* uuid, int page_addr);
};

struct nand_flash_dev_uid nand_flash_uids[] = {
	{"GD5F1GQ5REYIG SPI NAND 1G", { .id = {0xc8, 0x41} }, 2, 0x6, nandflash_uid_detect_common},
	{"F50D1G41LB(2M) SPI NAND 1G", { .id = {0xc8, 0x11} }, 2, 0x0, nandflash_uid_detect_common},
	{"GD5F1GM7REYIG SPI NAND 1G", { .id = {0xc8, 0x81} }, 2, 0x0, nandflash_uid_detect_common},
	{NULL}
};
...
void nandflash_uid_detect(struct nand_chip *chip)
{
	u8* nandflash_id;
	u8 uuid[129] = {0};
	...
	for (; type->name != NULL; type++)
	{
		if (type->id_len && !strncmp(type->id, nandflash_id, type->id_len))
		{
			...
			if (!type->flash_uid_detect(chip, uuid, type->page_addr))
			{
...
static int GD5F1GQ5_nandflash_uid_detect(struct nand_chip *chip, u8* uuid)
{
	struct qcom_nand_controller *nandc = get_qcom_nand_controller(chip);
	struct qcom_nand_host *host = to_qcom_nand_host(chip);
	int i = 0, status = 0, uid_error = 0, step = 0;
	int page_addr = 0x6;
...
//read uid step 1: Set Feature command to enable OTP_EN.
	...
	if (status < 0)
	{
		dev_err(nandc->dev,"Error in getting feature OTP_EN.\n");
		return status;
	}
	status = qcom_serial_set_feature(nandc, host, SPI_FLASH_FEATURE_REG,
				SPI_FLASH_OTP_ENABLE | (status >> 8));
	if (status < 0)
	{
		dev_err(nandc->dev,"Error in setting feature OTP_EN.\n");
		return status;
	}
...
//read uid step 2: Get Feature command and check if the OTP_EN is enable
	...
	if (!((status >> 8) & SPI_FLASH_OTP_ENABLE))
	{
		dev_err(nandc->dev,"OTP_EN is still disabled, setting error.\n");
		return -1;
	}

//read uid step 3: Page read to cache with page address 0x6, read data from page cache
	chip->legacy.cmdfunc(chip, NAND_CMD_READ0, 0x00, page_addr);
	status = qcom_nandc_read_page(chip, page_buf, 0, page_addr);
	if (status < 0)
	{
		dev_err(nandc->dev, "GD Flash Device read page for uid error\n");
		return status;
	}
...
//read uid step 5: read data to set uuid
	...
	for (i=0; i <16; i++)
	{
		...
		if ((page_buf[i] ^ page_buf[i+16]) != 0xFF)
		{
			uid_error = 1;
			break;
		}
	...
	if (uid_error==1)
	{
		pr_info("GD Flash Device read UID(0~15 bytes) error, read 32~47 bytes!\n");
		uid_error = 0;
		step = 32;
		for (i=32; i <48; i++)
		{
	...
	if (uid_error==1)
	{
		dev_err(nandc->dev, "GD Flash Device read UID error\n");
		return -1;
	}

	sprintf(uuid, "%02X%02X-", nandflash_id[0], nandflash_id[1]);

	for (i=step+0; i <step+16; i++)
	{
...
//read uid step 3: Page read to cache with page address 0x0, read data from page cache
	...
	if (status < 0)
	{
		dev_err(nandc->dev, "ESMT Flash Device read page for uid error\n");
		return status;
	}
...
//read uid step 5: read data to set uuid
	...
	if (uid_error==1)
	{
		dev_err(nandc->dev, "ESMT Flash Device read UID error\n");
		return -1;
	}
